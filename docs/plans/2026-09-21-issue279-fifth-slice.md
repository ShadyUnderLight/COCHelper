...
`perf:gate` 的输入不是任意两个 JSON，而是同一 exact head 的两组报告：
...
- numeric gate 当前只针对 `history-24`：总导入 p50、峰值 RSS、峰值 footprint；其他指标保留在报告中，但不自动成为本切片的 blocker。
...
candidate 只有在“不超过 reference 加相对 tolerance”且“reference 与 candidate 都不超过绝对上限”时才通过。contract 缺失、`status` 未冻结、tolerance/上限不是有限数值时，`perf:gate` 失败。
...
`origin/main@ad9ae2f` 的 Node 24 CI 已完成四个 Release perf job 并上传报告。现有 Node 26 文档数字仍是 observed evidence，不能替代当前 exact head 的 candidate reports，也不能单独形成 acceptance。
