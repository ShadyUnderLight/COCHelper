# 窄窗口：建筑组卡横向阶梯滚动、长中文名称换行（热缓存再次滚动）

> Issue #209 基线采样文档。静态信息已填，**指标字段待 Instruments 采样后填写**，
> 采样完成后删除「待采样」标注并回链 #196/#209。

## 操作步骤
1. 前置：冷启动采样（`05-narrow-window-cold.md`）已完成，窗口仍为 ~800pt，app 保持打开——数据处于热缓存状态。
2. 保持当前页（不离开建筑组卡），横向阶梯滚动，连续滚动 10 秒；覆盖长中文名称行。
3. 记录窄窗口布局下阶梯滚动与文字换行的主线程阻塞区间（热缓存段）；首次进入段见 `05-narrow-window-cold.md`，不得混入同一采样。
