...
/// - `parserVersion` 由各端点的约束扩展提供（如 `clan-snapshot-0.1`）。
public struct OfficialEndpointState<Snapshot: Codable & Hashable & Sendable>: Codable, Hashable, Sendable {
    ...
    public init(
        ...
        parserVersion: String = "endpoint-state",
...
public typealias ClanWarLogAPIState = OfficialEndpointState<OfficialPaginatedPage<OfficialWarLogEntry>>

/// 部落资本赛季状态（3c）：parserVersion "clan-capital-0.1"。
public typealias ClanCapitalAPIState = OfficialEndpointState<OfficialPaginatedPage<OfficialCapitalRaidSeason>>

// MARK: - 各端点 parserVersion（约束扩展）

public extension OfficialEndpointState where Snapshot == OfficialClanSnapshot {
    static var currentParserVersion: String { "clan-snapshot-0.1" }
}

public extension OfficialEndpointState where Snapshot == OfficialClanWarSnapshot {
    static var currentParserVersion: String { "clan-war-0.1" }
}

public extension OfficialEndpointState where Snapshot == OfficialPaginatedPage<OfficialWarLogEntry> {
    static var currentParserVersion: String { "clan-war-log-0.1" }
}

public extension OfficialEndpointState where Snapshot == OfficialPaginatedPage<OfficialCapitalRaidSeason> {
    static var currentParserVersion: String { "clan-capital-0.1" }
}
