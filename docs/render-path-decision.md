...
- 不修改已删除的迁移期 SwiftUI 页面；Electron renderer 的 UI 由 #277 负责
- 不提交 APK / 原始游戏资源包；/tmp 输出不入库（**渲染产物 PNG 例外**：随 private 仓库入库以满足 Electron Forge packaged resources，边界见 `docs/rendered-path-contract.md` R12.3）
