"""Task 7: validate_catalog 不变量（结构 + 语义业务校验）。测试是契约。
E0-03/Issue #303：不再校验 source SHA、generatedFiles hash/size、counts
对账与 manifest 登记（R-C）。"""
...
def test_validate_schema_version_mismatch_manifest(tmp_path):
    d = _valid_dir(tmp_path)
    m = _load_manifest(d)
    m["schemaVersion"] = 2  # 旧 schema 按 §WA-7.1 拒绝
    _write(d, manifest=m)
    errors = validate_catalog(d)
...
# ---- E0-03/Issue #303：counts / generatedFiles hash-size / R6.2 /
# sourceFingerprint 校验已整体删除（以下为删除占位，防行号漂移误读） ----
...
def _register_png(d: Path, rel_path: str, data: bytes) -> None:
    """写 PNG 文件（E0-03 起不再向 manifest 登记，仅落盘供 R-A 校验）。"""
...
def test_rendered_path_unregistered_ok(tmp_path):
    """R-C 已撤销：文件存在即通过，不再要求 manifest 登记。"""
    d = _valid_dir(tmp_path)
...
def test_rendered_path_version_segment_rejected_even_when_file_exists(tmp_path):
    """验收 7 缺口回归：icons/18.400.13/...（版本段）即使真实存在
...
def test_rendered_path_backslash_rejected_even_when_file_exists(tmp_path):
    ...
    也必须拒绝——反斜杠是 Windows 路径分隔符，跨平台逃逸风险。"""
    d = _valid_dir(tmp_path)
    png = b"\x89PNG\r\n\x1a\n" + b"w" * 8
    (d / "icons" / "ui").mkdir(parents=True)
    (d / "icons" / "ui" / "a_b.png").write_bytes(png)
    c = _load_catalog(d)
    # renderedPath 含反斜杠（未 sanitize 形态），文件指向正常路径
...
def test_rendered_path_dotdot_escape_rejected_without_probe(tmp_path):
    """NB-3 回归：icons/../../secret.png 指向 catalog 目录外的真实文件
    → 必须报格式非法，不得因逃逸探测命中而放行。"""
    d = _valid_dir(tmp_path)
...
def test_rendered_path_dotdot_inside_catalog_rejected(tmp_path):
    """目录内逃逸：icons/../x.png 解析到 catalog 根下的文件 → 格式非法。"""
    d = _valid_dir(tmp_path)
    png = b"\x89PNG\r\n\x1a\n" + b"x" * 8
