...
## 操作步骤
1. 账号数据页滚动全部卡片 10 秒（官方玩家卡、部落卡、战争卡、战争日志卡、突袭周末卡）。
2. war log / capital raid 多页：seed 已自动加载跟踪部落 #PERFCLAN 的 3 页 war log 与 3 页 raid 缓存（合并 30 / 17 条），打开对应卡片滚动分页列表。
3. 记录分页加载后的滚动 hitch 与图片解码内存（本文档对应 冷启动/首次进入）。
