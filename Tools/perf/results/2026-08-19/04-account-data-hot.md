# Account Data：账号摘要、官方玩家、部落、战争、war log、capital raid（热缓存再次滚动）

> Issue #209 基线采样文档。静态信息已填，**指标字段待 Instruments 采样后填写**，
> 采样完成后删除「待采样」标注并回链 #196/#209。

## 操作步骤
1. 前置：冷启动采样（`04-account-data-cold.md`）已完成，app 保持打开——数据处于热缓存状态。
2. 保持当前页（不离开账号数据页），滚动全部卡片 10 秒（官方玩家卡、部落卡、战争卡、战争日志卡、突袭周末卡）。
3. war log / capital raid 多页卡片滚动分页列表，分页加载与滚动行为单独记录。
4. 记录分页加载后的滚动 hitch 与图片解码内存（热缓存段）；首次进入段见 `04-account-data-cold.md`，不得混入同一采样。
