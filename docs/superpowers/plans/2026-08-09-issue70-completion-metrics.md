...
- currentStage 额外：known 但 `currentStageMaxLevel` 缺失（nil 或 ≤0）的实例计入阶段专用缺失权重 → partial（最终实现修订：升级中项可能 stageMax == nil——投影 isUpgrading 分支先于 stageMax 检查，真实可达；「known ⇒ stageMax 非 nil」只对非升级项成立；恶意目录 ≤0 cap 同样归缺失侧，不静默 0 贡献）。global 同理：nil/≤0 的非法 maxLevel 计入全局缺失权重 → partial（外部评审 P2-2）。
- **已观测分母（外部评审 P1-1，验收 3）**：`completeDenominator == false`（阶段 1 生产恒 false）且可计算 → stage/global 强制 `.partial`，原因文案置前「分母为已观测项目，非村庄全部实例，无法计算完整村庄进度。」——实例加权结果不得被误称全村庄进度；snapshotCoverage 不受影响（覆盖率语义天然是观测范围）。阶段 2 实例宇宙接入后传 true → ready 可达。
...
public enum VillageProgressProjection {
    ...
    public static func metrics(
        ...
        completeDenominator: Bool = false       // 阶段 1 无实例宇宙数据源，生产恒 false
...
> 已删除（2026-08-09 合并前清理）：本节为 Task 1 初版设计快照，与最终实现多处过时
> （旧版复制 isKnown、无 needsReimport 排除、无 stage/global 缺失权重、无负 cap 钳制、
> 无 completeDenominator）。实现全量代码以 Git 历史（a77d0e2 起）与最终源码为准；
> 本节删除避免误导。
