...
public struct CoAPIClient: Sendable {
    ...
    /// 拉取官方玩家信息。`tag` 传原始 tag（如 `#ABC`），URL 中的 `#` 由
    /// `CoAPIURLBuilder` 编码为 `%23`，调用方不要预先编码。
    public func fetchPlayer(tag: String) async throws -> OfficialPlayerSnapshot {
        let data = try await request(path: "/players/\(tag)")
        do {
            return try JSONDecoder().decode(OfficialPlayerSnapshot.self, from: data)
        } catch let error as DecodingError {
            // 附上字段路径（仅键名，不含 URL/token/正文），便于定位 schema 漂移；
            // 纯语法错误（路径为空）保持原样。
            if let path = decodingPath(of: error) {
                throw CoAPIError.malformedResponse(detail: "player decode failed: \(path)")
            }
            throw CoAPIError.malformedResponse(detail: "player decode failed")
        } catch {
            throw CoAPIError.malformedResponse(detail: "player decode failed")
        }
    }
    ...
    /// 拉取官方部落当前战争。`tag` 传原始 tag（如 `#ABC`），URL 中的 `#` 由
    /// `CoAPIURLBuilder` 编码为 `%23`，调用方不要预先编码。
    public func fetchClanWar(tag: String) async throws -> OfficialClanWarSnapshot {
        let data = try await request(path: "/clans/\(tag)/currentwar")
        do {
            return try JSONDecoder().decode(OfficialClanWarSnapshot.self, from: data)
        } catch let error as DecodingError {
            if let path = decodingPath(of: error) {
                throw CoAPIError.malformedResponse(detail: "clan war decode failed: \(path)")
            }
    ...
    private func decodingPath(of error: DecodingError) -> String? {
        ...
        let keys = path.map { $0.stringValue }
