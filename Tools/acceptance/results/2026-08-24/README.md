...
| A/B 串档 + lineage 隔离 | **待本地数据** | runner 输出 `real-village-acceptance.json`，含 `after-restart` 两边 SanitizedVillageHistory + 最终 `totalSnapshotCount 2/2` |
...
| Release App 人工性能（导入/Diff/重启） | **待人工** | 见 [performance-large-walls.md](performance-large-walls.md)（**两步粘贴 before→after** 同村同 tag 产生确定性大量 Wall Diff；单次粘贴仅 baseline 无法复现场景 3） |
