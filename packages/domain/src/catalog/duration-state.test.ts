...
describe('catalogDurationState', () => {
  it('映射 timed/instant/缺失类语义', () => {
    ...
    expect(catalogDurationState(null, 'min_level_initial_no_upgrade')).toEqual({ kind: 'initialLevel' });
    ...
    expect(catalogDurationState(-1n, null)).toEqual({ kind: 'unknownReason', reason: 'negative_duration' });
  });
