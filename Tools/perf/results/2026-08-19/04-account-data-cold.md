# Account Data：账号摘要、官方玩家、部落、战争、war log、capital raid（冷启动/首次进入）

> Issue #209 基线采样文档。静态信息已填，**指标字段待 Instruments 采样后填写**，
> 采样完成后删除「待采样」标注并回链 #196/#209。

## 操作步骤
1. 账号数据页冷启动后打开，滚动全部卡片 10 秒（官方玩家卡、部落卡、战争卡、战争日志卡、突袭周末卡）。
2. war log / capital raid 多页：seed 已自动加载跟踪部落 #PERFCLAN 的 3 页 war log 与 3 页 raid 缓存（合并 30 / 17 条），打开对应卡片滚动分页列表，分页加载与滚动行为单独记录。
3. 记录分页加载后的滚动 hitch 与图片解码内存（首次进入段）；热缓存段见 `04-account-data-hot.md`，不得混入同一采样。
