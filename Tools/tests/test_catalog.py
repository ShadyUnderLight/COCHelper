...
# ---- E0-03/Issue #303：manifest 精简为四字段版本元数据 ----

def test_generate_manifest_is_minimal_four_fields(full_minimal_apk, tmp_path):
    """生成器产出的 manifest 只含 schemaVersion/gameVersion/buildTag/locale；
    无 sourceFingerprint / generatedFiles / counts。"""
    import json
    out = tmp_path / "out"
    generate(full_minimal_apk, None, out)
    m = json.loads((out / "manifest.json").read_text())
    assert m["schemaVersion"] == 3
    assert m["gameVersion"]
    assert m["buildTag"]
    assert m["locale"] == "zh-CN"
    assert "sourceFingerprint" not in m
    assert "generatedFiles" not in m
    assert "counts" not in m
