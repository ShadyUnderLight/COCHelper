...
- 账号快照：`Tests/COCHelperCoreTests/Fixtures/perf_account_snapshot_home.json`、
  `perf_account_snapshot_builder.json`、`perf_account_snapshot_mixed.json`
...
1. Release App 启动 → 账号数据页粘贴 `perf_account_snapshot_home.json` → 确认导入。
...
1. 同场景 1 导入，进入详情页。
2. 切换基地（主村 ↔ 建筑工人基地，`perf_account_snapshot_builder.json` / `_mixed.json` 覆盖）。
3. 切换搜索/状态筛选；展开/收起快照历史与手动队列面板。
4. 每次切换后滚动 10 秒，记录 tick 与投影调用变化。

## 场景 3：Upgrade Overview 多村庄、manual 状态、各面板
1. 依次导入 3 个账号 fixture（home / builder / mixed），在总览页打开。
2. 通过行内 Start 建立若干 manual active（≥5 项），等待/触发 settle 建立 completed。
3. 滚动 active / pending / recently completed / attention 各面板各 10 秒。
