...
## 操作步骤
1. 窗口缩到 ~800pt 宽（windowResizability(.contentSize) 允许），冷启动后打开建筑组卡（首次进入）。
2. 连续滚动 10 秒；覆盖长中文名称行。
3. 记录窄窗口布局下阶梯滚动与文字换行的主线程阻塞区间（首次进入段）；热缓存段见 `05-narrow-window-hot.md`，不得混入同一采样。
