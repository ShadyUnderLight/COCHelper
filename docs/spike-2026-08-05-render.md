...
- **测试**：`python3 -m pytest Tools/tests -q` → **294 passed**（交叉审核后实测），含 hypothesis property-based（`is_renderable` 一致性 + R5.2 互斥不变量）。
...
| 可复核性 | 复跑命令：`cd Tools && python3 render_spike.py --apk /Users/lmz/Downloads/base.apk.1 --output /tmp/coc-spike-report-check`；契约测试：`python3 -m pytest Tools/tests -q`（294 passed） |
