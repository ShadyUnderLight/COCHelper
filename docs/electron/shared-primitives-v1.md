...
`StableId` 由语义组件按 Swift `TrackerItemKey.stableID` 的 `|` 分隔规则构造，是
deterministic printable stable ID；组件只能是字符串或 `bigint`，不允许数组位置、
展示名称或本地化文本进入身份。它不替代 Swift 契约中的 structural persisted identity。
`LineageId` 是带独立语义 brand 的大写连字符 UUID，必须由 `UuidSource` 注入或从已校验
文本解析。
